...
mixin TieSheetLayoutHelperMixin {
  ...
  String participantDisplayName(ParticipantEntity p) => p.fullName.toUpperCase();
  ...
  ParticipantRowLayoutData computeParticipantRowLayout({
    ...
    final boldWeight = themeConfig.isTextForceBold ? FontWeight.w900 : FontWeight.bold;
    final normalWeight = themeConfig.isTextForceBold ? FontWeight.w800 : FontWeight.normal;

    if (!isMirrored) {
      accentRect = Rect.fromLTRB(x, y + 1, x + themeConfig.accentStripWidth, y + rowH - 1);
      dividers = [
        LineSegmentLayoutData(startOffset: Offset(x + noColW, y + 4), endOffset: Offset(x + noColW, y + rowH - 4)),
        LineSegmentLayoutData(startOffset: Offset(x + noColW + nameColW, y + 4), endOffset: Offset(x + noColW + nameColW, y + rowH - 4)),
      ];
      serialText = PositionedTextLayoutData(
        ...
        textColorType: isPlaceholder ? TextColorType.muted : TextColorType.secondary,
      );
      nameText = PositionedTextLayoutData(
        ...
        textColorType: isPlaceholder ? TextColorType.muted : TextColorType.primary,
      );
      ...
      accentRect = Rect.fromLTRB(right - themeConfig.accentStripWidth, y + 1, right, y + rowH - 1);
      dividers = [
        LineSegmentLayoutData(startOffset: Offset(x + regIdColW, y + 4), endOffset: Offset(x + regIdColW, y + rowH - 4)),
        LineSegmentLayoutData(startOffset: Offset(x + regIdColW + nameColW, y + 4), endOffset: Offset(x + regIdColW + nameColW, y + rowH - 4)),
      ];
      serialText = PositionedTextLayoutData(
        textContent: '$serialNumber',
  ...
  int computeParticipantListLayout({
    ...
    for (final match in roundOneMatches) {
      final blueParticipant = findParticipantById(match.participantBlueId, participants);
      final redParticipant = findParticipantById(match.participantRedId, participants);
      ...
      if (drawBlue) {
        ...
        final name = isPlaceholder ? 'TBD' : participantDisplayName(blueParticipant);
        outputRows.add(computeParticipantRowLayout(
          matchId: match.id, slotPosition: 'blue',
          participantId: match.participantBlueId, serialNumber: idx,
          x: x, y: y, dims: dims, themeConfig: themeConfig,
          isMirrored: isMirrored, isPlaceholder: isPlaceholder,
          displayName: name,
          displayRegistrationId: blueParticipant?.registrationId,
        ));
        nodeOffsets['${match.id}_top_input'] = Offset(nodeX, y + dims.participantRowHeight / 2);
      ...
      if (drawRed) {
        ...
        final name = isPlaceholder ? 'TBD' : participantDisplayName(redParticipant);
        outputRows.add(computeParticipantRowLayout(
          matchId: match.id, slotPosition: 'red',
          participantId: match.participantRedId, serialNumber: idx,
          x: x, y: y, dims: dims, themeConfig: themeConfig,
          isMirrored: isMirrored, isPlaceholder: isPlaceholder,
          displayName: name,
          displayRegistrationId: redParticipant?.registrationId,
        ));
        nodeOffsets['${match.id}_bot_input'] = Offset(nodeX, y + dims.participantRowHeight / 2);
        y += dims.participantRowHeight;
      }
  ...
  Offset? resolveInputOffset({
    ...
    final explicitKey = isTopSlot ? '${match.id}_top_input' : '${match.id}_bot_input';
    if (nodeOffsets.containsKey(explicitKey)) return nodeOffsets[explicitKey];

    final wFeeders = allMatches.where((m) => m.winnerAdvancesToMatchId == match.id).toList();
    final lFeeders = allMatches.where((m) => m.loserAdvancesToMatchId == match.id).toList();
    ...
    if (wFeeders.isNotEmpty && lFeeders.isEmpty) {
      if (wFeeders.length == 1) {
        ...
        wFeeders.sort((a, b) => a.matchNumberInRound.compareTo(b.matchNumberInRound));
        if (match.bracketId != winnersBracketId && match.bracketId != losersBracketId) {
          final wbF = wFeeders.where((f) => f.bracketId == winnersBracketId).firstOrNull;
          final lbF = wFeeders.where((f) => f.bracketId == losersBracketId).firstOrNull;
          targetFeeder = isTopSlot ? (wbF ?? wFeeders.first) : (lbF ?? wFeeders.last);
        } else {
      ...
      if (lFeeders.length == 1) {
        ...
        lFeeders.sort((a, b) => a.matchNumberInRound.compareTo(b.matchNumberInRound));
    ...
    if (targetFeeder != null) {
      final isCrossBracket = targetFeeder.bracketId == winnersBracketId &&
  ...
  void computeJunctionLayout({
    ...
    if (isBye) {
      _computeByeJunction(match, junctionX, isMirrored, allMatches,
          dims, themeConfig, nodeOffsets, matchLayouts, connectors,
          winnersBracketId, losersBracketId);
      return;
    }

    final topIn = resolveInputOffset(match: match, isTopSlot: true,
        allMatches: allMatches, nodeOffsets: nodeOffsets,
        winnersBracketId: winnersBracketId, losersBracketId: losersBracketId);
    final botIn = resolveInputOffset(match: match, isTopSlot: false,
        allMatches: allMatches, nodeOffsets: nodeOffsets,
        winnersBracketId: winnersBracketId, losersBracketId: losersBracketId);
    ...
    final topArcType = match.participantBlueId != null
        ? ConnectorVisualType.wonAdvancement : ConnectorVisualType.pendingAdvancement;
    final botArcType = match.participantRedId != null
        ? ConnectorVisualType.wonAdvancement : ConnectorVisualType.pendingAdvancement;

    connectors.add(ConnectorLayoutData.arcPath(
      connectorVisualType: topArcType,
      pathData: ConnectorArcPathData(
        moveToOffset: effectiveTop,
        lineToBeforeArcOffset: Offset(junctionX - (isMirrored ? -r : r), effectiveTop.dy),
        arcEndOffset: Offset(junctionX, effectiveTop.dy + r),
        arcRadius: r, isArcClockwise: !isMirrored,
    ...
    connectors.add(ConnectorLayoutData.arcPath(
      ...
      pathData: ConnectorArcPathData(
        ...
        lineToBeforeArcOffset: Offset(junctionX - (isMirrored ? -r : r), effectiveBot.dy),
    ...
    final badgeRadius = max(themeConfig.badgeMinRadius,
        max(computeFontSize(9, delta), computeFontSize(9, delta)) / 2 + themeConfig.badgePadding);

    MatchNumberPillLayoutData? pillLayout;
    if (gNum != null) {
      final pillHalfW = max(themeConfig.matchPillMinHalfWidth, 8.0 + themeConfig.matchPillHorizontalPadding);
      final pillHalfH = max(themeConfig.matchPillMinHalfHeight, 8.0 + themeConfig.matchPillVerticalPadding);
      final pillRect = Rect.fromCenter(center: Offset(junctionX, midY), width: pillHalfW * 2, height: pillHalfH * 2);
      pillLayout = MatchNumberPillLayoutData(
        centerOffset: Offset(junctionX, midY), matchNumberText: '$gNum',
    ...
    if (missingTopLabel != null) {
      ...
      missingTopText = PositionedTextLayoutData(
        ...
        fontSize: computeFontSize(9, delta), fontWeight: FontWeight.bold,
        fontStyle: FontStyle.italic, isRightAligned: !isMirrored,
    ...
    matchLayouts.add(MatchLayoutData(
      matchId: match.id, matchNodeType: MatchNodeType.standardJunction,
      isByeMatch: false, blueCornerBadgeLayout: CornerBadgeLayoutData(
        centerOffset: Offset(badgeX, effectiveTop.dy - 14), badgeText: 'B',
        badgeColorType: CornerBadgeColorType.blue, computedBadgeRadius: badgeRadius,
      ), redCornerBadgeLayout: CornerBadgeLayoutData(
        centerOffset: Offset(badgeX, effectiveBot.dy + 14), badgeText: 'R',
        badgeColorType: CornerBadgeColorType.red, computedBadgeRadius: badgeRadius,
      ), matchNumberPillLayout: pillLayout, winnerNameTextLayout: winnerText,
  ...
  void _computeByeJunction(
    MatchEntity match, double junctionX, bool isMirrored,
    List<MatchEntity> allMatches, TieSheetLayoutDimensions dims,
    ...
    final topType = match.participantBlueId != null
        ? ConnectorVisualType.wonAdvancement : ConnectorVisualType.pendingAdvancement;
    connectors.add(ConnectorLayoutData.singleLine(
      connectorVisualType: topType,
      startOffset: topIn, endOffset: Offset(junctionX, topIn.dy),
    ));

    final nextJunctionX = isMirrored ? junctionX - dims.roundColumnWidth : junctionX + dims.roundColumnWidth;
    connectors.add(ConnectorLayoutData.singleLine(
      connectorVisualType: ConnectorVisualType.byeAdvancement,
      startOffset: Offset(junctionX, topIn.dy), endOffset: Offset(nextJunctionX, topIn.dy),
    ));
    nodeOffsets['${match.id}_output'] = Offset(nextJunctionX, topIn.dy);

    matchLayouts.add(MatchLayoutData(
      matchId: match.id, matchNodeType: MatchNodeType.standardJunction, isByeMatch: true,
    ));
  }
}
