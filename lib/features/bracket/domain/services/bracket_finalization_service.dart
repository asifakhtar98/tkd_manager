...
class BracketFinalizationService {
  ...
  BracketResult finalizeBracket(BracketResult result) {
    return result.map(
      singleElimination: (s) {
        ...
        return BracketResult.singleElimination(s.data.copyWith(
      ...
      doubleElimination: (d) {
        ...
        return BracketResult.doubleElimination(d.data.copyWith(
  ...
  List<BracketMedalPlacementEntity> _calculateSingleElimination(BracketGenerationResult data) {
    ...
    if (finals != null && finals.winnerId != null) {
      placements.add(BracketMedalPlacementEntity(participantId: finals.winnerId!, rankStatus: 1, displayPlacementLabel: '1st'));
      final silverId = finals.winnerId == finals.participantRedId ? finals.participantBlueId : finals.participantRedId;
      if (silverId != null) {
        placements.add(BracketMedalPlacementEntity(participantId: silverId, rankStatus: 2, displayPlacementLabel: '2nd'));
      }
    }
    ...
    if (thirdM != null) {
      if (thirdM.winnerId != null) {
        placements.add(BracketMedalPlacementEntity(participantId: thirdM.winnerId!, rankStatus: 3, displayPlacementLabel: '3rd'));
        final loserId = thirdM.winnerId == thirdM.participantRedId ? thirdM.participantBlueId : thirdM.participantRedId;
        if (loserId != null) {
          placements.add(BracketMedalPlacementEntity(participantId: loserId, rankStatus: 4, displayPlacementLabel: '3rd'));
        }
      }
    } else {
      // Award bronze to both semi-final losers
      if (allRounds > 1) {
        final semiFinals = matches.where((m) => m.roundNumber == allRounds - 1).toList();
        for (final semi in semiFinals) {
          if (semi.winnerId != null) {
            final loserId = semi.winnerId == semi.participantRedId ? semi.participantBlueId : semi.participantRedId;
            if (loserId != null) {
              placements.add(BracketMedalPlacementEntity(
  ...
  List<BracketMedalPlacementEntity> _calculateDoubleElimination(DoubleEliminationBracketGenerationResult data) {
    final placements = <BracketMedalPlacementEntity>[];
    
    // Grand Finals (consider Reset Match if it exists and has a winner)
    final finalMatchInfo = (data.resetMatch != null && data.resetMatch!.winnerId != null) 
        ? data.resetMatch! 
        : data.grandFinalsMatch;

    if (finalMatchInfo.winnerId != null) {
      placements.add(BracketMedalPlacementEntity(participantId: finalMatchInfo.winnerId!, rankStatus: 1, displayPlacementLabel: '1st'));
      final silverId = finalMatchInfo.winnerId == finalMatchInfo.participantRedId ? finalMatchInfo.participantBlueId : finalMatchInfo.participantRedId;
      if (silverId != null) {
        placements.add(BracketMedalPlacementEntity(participantId: silverId, rankStatus: 2, displayPlacementLabel: '2nd'));
      }
    }
    ...
    final losersMatches = data.allMatches.where((m) => m.bracketId == data.losersBracket.id).toList();
    if (losersMatches.isNotEmpty) {
      int maxLoserRound = 1;
      for (final m in losersMatches) {
        if (m.roundNumber > maxLoserRound) maxLoserRound = m.roundNumber;
      }
      final losersFinal = losersMatches.where((m) => m.roundNumber == maxLoserRound && m.matchNumberInRound == 1).firstOrNull;
      if (losersFinal != null && losersFinal.winnerId != null) {
         final bronzeId = losersFinal.winnerId == losersFinal.participantRedId ? losersFinal.participantBlueId : losersFinal.participantRedId;
         if (bronzeId != null) {
            placements.add(BracketMedalPlacementEntity(participantId: bronzeId, rankStatus: 3, displayPlacementLabel: '3rd'));
         }
      }
    }
