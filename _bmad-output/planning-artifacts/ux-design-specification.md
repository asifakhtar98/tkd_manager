...
- **Desktop (1280px+):** Command Center. Dense layout with bracket canvas, keyboard shortcuts available.
- **Tablet (768px-1024px):** Optimized single-pane view, larger touch targets for scoring.
- **Mobile (320px-767px):** Read-Only spectator view (planned for Phase 2).
...
- **Latency:** < 300ms for bracket generation feedback (internet-dependent).
- **Data Persistence:** Tournament data is stored in Supabase cloud. UI state (bracket format, participant list) is preserved locally via HydratedBloc.
- **Forgiveness:** Global `Undo` instead of destructive `Confirm` modals.
- **Accessibility:** WCAG AA. Minimum 4.5:1 text contrast for all text.
